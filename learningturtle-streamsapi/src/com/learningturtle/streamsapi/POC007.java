...
public class POC007 {
	...
	public static void main(String[] args) {
		...
		/* Ignore Case Result */
		String s = text.chars() // IntStream : Stream of surrogate codepoint of the Chars
				...
				.filter(x -> x.getValue() == 1) // Filter all the element with Count 1
		...
		s = text.chars() // IntStream : Stream of surrogate codepoint of the Chars
				.mapToObj(n -> (char) n) // Convert the surrogate codepoint into chars, It will return Stream of Characters
				.collect(Collectors.groupingBy(Function.identity(), LinkedHashMap::new, Collectors.counting())) // It will create a LinkedHashMap with Char as Key and Char Count as Value
				.entrySet() // Get the Entry Set from the Map
				.stream() // Create a Stream again to get the First Element with Count == 1
				.filter(x -> x.getValue() == 1) // Filter all the element with Count 1
